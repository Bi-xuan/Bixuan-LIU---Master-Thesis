...
Lambda_1 <- list(Lambda_init[[5]])
Lambda_2 <- list(Lambda_init[[5]])

# Define the candidate values for the penalty parameter
gma_cand <- log(seq(1, 100, 0.1), 2)
gma <- c(gma_cand[1])

# --- First run to get an initial solution ---
# Call ln_ADMM instead of ft_ISTA_bktr
res_ln_ADMM <- ln_ADMM(Lambda_1[[1]], Lambda_2[[1]], n, Sigma, omega, gma[1])

# Store the results from the ADMM output list
Lambda_1[[1]] <- res_ln_ADMM[[1]]
Lambda_2[[1]] <- res_ln_ADMM[[2]]
obj <- list(res_ln_ADMM[[4]]) # Objective value is the 4th element

k <- 1

# --- Main loop to find changes in sparsity ---
for (i in 1:100) {
  # Initialize the next step with the result of the previous step
  Lambda_1[[i+1]] <- Lambda_1[[i]]
  Lambda_2[[i+1]] <- Lambda_2[[i]]
  
  # This while loop keeps increasing 'gma' until the sparsity pattern of Lambda_1 changes
  while (all((Lambda_1[[i]] != 0) == (Lambda_1[[i+1]] != 0))) {
    k <- k + 1
    if (k > length(gma_cand)) break # Prevent running out of candidates
    
    t_gma <- gma_cand[k]
    
    # Call ln_ADMM using the previous solution as a "warm start"
    res_ln_ADMM <- ln_ADMM(Lambda_1[[i]], Lambda_2[[i]], n, Sigma, omega, t_gma)
    
    # Update the next-step matrices with the new result
    Lambda_1[[i+1]] <- res_ln_ADMM[[1]]
    Lambda_2[[i+1]] <- res_ln_ADMM[[2]]
  }
  
  # Store the objective and the 'gma' value that caused the pattern to change
  obj[[i+1]] <- res_ln_ADMM[[4]]
  gma[i+1] <- t_gma
  
  # Break if we've used all candidates or if the matrix becomes dense
  # Note: The sparsity check is on Lambda_1, which is subject to the L1 penalty.
  if (k >= length(gma_cand) || nnzero(Lambda_1[[i+1]]) == n) break
}
...
IHT
```{r}
Lambda <- list(Lambda_init[[10]])

eta <- 1
eta_inc <- 1.05
# gma_cand <- c(0,log(seq(100,1,-0.1),10))
# gma_cand <- c(0,10 ** (seq(-6,log10(5),0.01)))
gma_cand <- log(seq(1,100,0.1),10)
gma <- c(gma_cand[1])
# gma_cand <- 10 ** (seq(-5,log10(2.5),0.003))
# gma_cand <- seq(0,2,0.001)
...
# for (k in 1:(length(Lambda_init)-1)) {
for (k in 1:1) {
  ...
  df_t <- data.frame(lmd = array(Lambda_2), gma = gma[1:length(obj)], obj = array(unlist(obj)), init_id = rep(as.character(k),length(obj)))
...
for (k in 1:1) {
  ...
  df_t <- data.frame(lmd = array(Lambda_1), gma = gma[1:length(obj)], obj = array(unlist(obj)), init_id = rep(as.character(k),length(obj)))
...
# for (k in 1:length(Lambda_init)) {
  ...
  df_t <- data.frame(lmd = array(Lambda), gma = gma[1:length(obj)], obj = array(unlist(obj)), init_id = rep(as.character(10),length(obj)))
...
df_7 <- subset(df_long, init_id == "10" & row != col) 
...
ggplot(df_7, aes(x = gma, y = value, 
  ...
  # scale_x_log10() +
  labs(
    title = "Lambda entries vs Gamma (Initialization 10)",
    # x = expression(log_10(gamma)),
    x = expression(gamma),
...
ROC
```{r}
# --- 1. Define True Support ---
Lambda_supp <- (Lambda_0 != 0)

# --- 3. Process Data, Keeping init_id ---
roc_data_by_init <- df %>%
  rowwise() %>%
  mutate(
    lmd_tidy = list(
      as.data.frame.table(lmd, responseName = "value") %>%
        mutate(Var1 = as.integer(Var1), Var2 = as.integer(Var2))
    )
  ) %>%
  ungroup() %>%
  select(init_id, lmd_tidy) %>% # Keep init_id
  unnest(lmd_tidy) %>%
  filter(Var1 != Var2) %>%
  group_by(init_id, Var1, Var2) %>% # Group by init_id as well
  summarise(
    score = max(abs(value), na.rm = TRUE),
    .groups = 'drop'
  )

# Prepare true labels for joining
true_labels <- as.data.frame.table(Lambda_supp, responseName = "is_true") %>%
  mutate(
...
# Join scores with true labels
roc_data_by_init <- roc_data_by_init %>%
  left_join(true_labels, by = c("Var1", "Var2"))

# --- 4. Manually Calculate ROC and AUC for Each Group ---

# This function calculates ROC points for a single group of data
calculate_roc <- function(data) {
  # Order by score for this group
  data <- data %>% arrange(desc(score))
  
  P <- sum(data$is_true)
  N <- sum(data$is_true == 0)
  
  # Handle cases where there are no positives or no negatives to avoid division by zero
  if (P == 0 || N == 0) return(data.frame(FPR = numeric(), TPR = numeric()))
  
  TPs <- cumsum(data$is_true)
  FPs <- cumsum(data$is_true == 0)
  
  roc_pts <- data.frame(FPR = FPs / N, TPR = TPs / P)
  
  # Add the starting (0,0) point
  return(rbind(data.frame(FPR = 0, TPR = 0), roc_pts))
}

# Apply the function to each init_id group
roc_points_by_init <- roc_data_by_init %>%
  group_by(init_id) %>%
  group_modify(~ calculate_roc(.x)) %>%
  ungroup()

# Now, calculate the AUC for each init_id
auc_summary <- roc_points_by_init %>%
  group_by(init_id) %>%
  summarise(
    auc = sum( (TPR[-1] + TPR[-length(TPR)]) * diff(FPR) / 2 ),
    .groups = 'drop'
  )

# Print the summary table of AUC values
print("AUC for each initialization:")
print(auc_summary)

# --- 5. Plot All ROC Curves on One Graph ---
ggplot(roc_points_by_init, aes(x = FPR, y = TPR, color = init_id, group = init_id)) +
  geom_line(alpha = 0.8, size = 1) + # Draw one line for each init_id
  geom_segment(aes(x = 0, xend = 1, y = 0, yend = 1), color="grey", linetype="dashed", inherit.aes = FALSE) +
  labs(
    title = "ROC Curves per Initialization",
    subtitle = "Assessing model stability across different starting points",
...
```{r}
ADMM_supp <- function(n,Sigma,omega,Lambda_t,beta=0.5) {
  for (j in 1:5) {
    # Initialization
    Lambda_1 <- list(matrix(runif(n**2,-1,1),ncol=n))
    Lambda_2 <- list(matrix(runif(n**2,-1,1),ncol=n))
    ...
    for (i in 1:1000) {
      ...
      if(dis_1_2[[i+1]] < 1e-6 & obj[[i+1]] < 1e-6) {
        return(list(Lambda_1[[i+1]],obj[[i+1]],TRUE)) # Return the converged \Lambda and the objective function value
      }
    }
  }
  return(list(Lambda_1[[i+1]],obj[[i+1]],FALSE))
}
```
...
Ver.1
```{r}
ADMM_iteration <- function(Lambda_1,Lambda_2,alpha,Sigma,omega,beta=0.5) {
  n <- nrow(Sigma)
  
  Lambda_1_updt <- solve(2*(Sigma %*% Lambda_2 %*% t(Lambda_2) %*% Sigma) + beta * diag(n)) %*% (2*(Sigma %*% Lambda_2 %*% (Sigma - omega * diag(n))) - alpha + beta * Lambda_2)
  Lambda_2_updt <- solve(2*Sigma %*% Lambda_1_updt %*% t(Lambda_1_updt) %*% Sigma + beta * diag(n)) %*% (2 * Sigma %*% Lambda_1_updt %*% (Sigma - omega * diag(n)) + alpha + beta * Lambda_1_updt)
  alpha_updt <- alpha + beta * (Lambda_1_updt - Lambda_2_updt)
  
  return(list(Lambda_1_updt,Lambda_2_updt,alpha_updt))
}

ADMM_ISTA_update <- function(Lambda_1,Lambda_2,alpha,Sigma,omega,gma,eta) {
  n <- nrow(Sigma)
  # ADMM iteration
  res_ADMM <- ADMM_iteration(Lambda_1,Lambda_2,alpha,Sigma,omega)
  Lambda_1_ADMM <- res_ADMM[[1]]
  Lambda_2_ADMM <- res_ADMM[[2]]
  alpha_updt <- res_ADMM[[3]]
  ...
  Lambda_1_updt[!mask_diag & Lambda_1_ADMM < -eta*gma] <- Lambda_1_ADMM[!mask_diag & Lambda_1_ADMM < -eta*gma] + eta*gma
  Lambda_1_updt[!mask_diag & Lambda_1_ADMM >  eta*gma] <- Lambda_1_ADMM[!mask_diag & Lambda_1_ADMM >  eta*gma] - eta*gma
  Lambda_1_updt[!mask_diag & abs(Lambda_1_ADMM) <= eta*gma] <- 0
  
  Lambda_2_updt[!mask_diag & Lambda_2_ADMM < -eta*gma] <- Lambda_2_ADMM[!mask_diag & Lambda_2_ADMM < -eta*gma] + eta*gma
  Lambda_2_updt[!mask_diag & Lambda_2_ADMM >  eta*gma] <- Lambda_2_ADMM[!mask_diag & Lambda_2_ADMM >  eta*gma] - eta*gma
  Lambda_2_updt[!mask_diag & abs(Lambda_2_ADMM) <= eta*gma] <- 0
...
ln_ADMM <- function(n,Sigma,omega,gma,L_1=83,L_2=5,beta=75){
  ...
  for (i in 1:1000) {
    # Update Lambda_1
    # Gradient step
    Lambda_1_grad <- Lambda_1[[i]] - (-2 *(Sigma %*% Lambda_2[[i]]) %*%(Sigma - t(Lambda_2[[i]]) %*% Sigma %*% Lambda_1[[i]] - omega * diag(n)) + alpha[[i]] + beta * (Lambda_1[[i]] - Lambda_2[[i]])) / L_1
    ...
    # Update Lambda_2
    Lambda_2[[i+1]] <- (L_2 * Lambda_2[[i]] + alpha[[i]] + beta * Lambda_1[[i+1]] + 2 * (Sigma %*% Lambda_1[[i+1]]) %*% (Sigma - t(Lambda_1[[i+1]]) %*% Sigma %*% Lambda_2[[i]] - omega * diag(n))) / (L_2 + beta)
    ...
    dis_1_2[[i+1]] <- frobenius.norm(Lambda_1[[i+1]] - Lambda_2[[i+1]])
    obj[[i+1]] <- frobenius.norm(Sigma - t(Lambda_1[[i+1]]) %*% Sigma %*% Lambda_2[[i+1]] - omega * diag(n)) **2 + gma * (sum(abs(c(Lambda_1[[i+1]]) - c(diag(diag(Lambda_1[[i+1]]))))))
    obj_real[[i+1]] <- frobenius.norm(Sigma - t(Lambda_1[[i+1]]) %*% Sigma %*% Lambda_2[[i+1]] - omega * diag(n)) **2
