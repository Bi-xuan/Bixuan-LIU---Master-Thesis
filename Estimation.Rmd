...
eta_inc <- 1.5
gma <- seq(0,10,0.01)
obj <- list(frobenius.norm(Sigma - t(Lambda[[1]])%*%Sigma%*%Lambda[[1]] - omega * diag(n)) ** 2)
err_edges <- list(sum((Lambda_0 != 0) != (Lambda[[1]] != 0)))
...
```{r}
df <- data.frame(lmd = array(Lambda), gma = gma[1:length(obj)], obj = array(unlist(obj)), err_edges = array(unlist(err_edges)), init = rep("NEIB",length(obj)))
...
df_t <- data.frame(lmd = array(Lambda), gma = gma[1:length(obj)], obj = array(unlist(obj)), err_edges = array(unlist(err_edges)), init = rep("ADMM",length(obj)))

df_t_plt <- df_t[-1,]
df <- rbind(df, df_t)
df_plt <- rbind(df_plt,df_t_plt)
```

```{r}
# Obj
plt_1 <- ggplot(df_plt,aes(x=gma,y=obj,color=init)) + 
  geom_line() + 
  labs(x="Gamma",y="Objective value")

plt_2 <- ggplot(df_plt,aes(x=gma,y=obj,color=init)) + 
  geom_point() + 
  labs(x="Gamma",y="Objective value")
...
# n=5
n <- 5
Lambda_supp <- rbind(c(1,1,0,0,0),c(0,1,1,0,0),c(0,0,1,1,0),c(0,0,0,1,1),c(0,0,0,0,1))
