...
ft_ISTA_bktr <- function(Lambda_0,Sigma,gma,eta_0) {
  # eta_0 is the initial stepsize (before backtracking)
  eta <- list(eta_0)
  eta_inc <- 1.1 # The inverse of increment of stepsize, a constant > 1
  
  Lambda <- list(Lambda_0)
  obj <- list(frobenius.norm(Sigma - t(Lambda_0)%*%Sigma%*%Lambda_0 - omega * diag(n)) ** 2 + gma * sum(abs(c(Lambda_0) - c(diag(diag(Lambda_0))))))
  obj_gap <- list(obj[[1]])
  ...
  print("ISTA did not converge within 1000 iterations.")
  return(list(Lambda[[i+1]],obj[[i+1]]))
}
```
...
```{r}
n <- 3
# Lambda_0 <- rbind(c(runif(1,-1,1),0,runif(1,-1,1)),c(0,runif(1,-1,1),runif(1,-1,1)),c(runif(1,-1,1),runif(1,-1,1),runif(1,-1,1)))
...
for (i in 1:100) {
  ...
  while (all((Lambda[[i]]!=0) == (Lambda[[i+1]]!=0))) {
    t_gma <- t_gma + 0.01
    res_ISTA_bktr <- ft_ISTA_bktr(Lambda[[i+1]],Sigma,t_gma,eta)
